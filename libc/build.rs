// libc compiles c-ares (the leading-standard async DNS stub resolver, MIT) so
// that getaddrinfo(3) is backed by a real resolver system-wide. The c-ares
// objects land in liboxbow_libc; with -ffunction-sections + --gc-sections, a
// program that never calls getaddrinfo (hello, cat, tcc…) drops them entirely,
// so only resolvers (curl, cares-test) pay the size. Mirrors the lwext4/BearSSL
// C-port harness; the OS-config-file + event-thread backends are excluded and
// their few symbols stubbed in cares_glue.c.
...
fn main() {
    ...
    // Platform / event-backend sources oxbow doesn't have (symbols stubbed in
    // cares_glue.c).
    let skip = [
        "ares_android.c",
