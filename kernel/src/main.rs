...
fn kmain_stage2() -> ! {
    ...
    for file in mods.iter() {
        ...
        if matches!(
            cmd,
            b"pong" | b"beta" | b"hello" | b"badge" | b"cat" | b"ls" | b"mkdir" | b"touch" | b"rm" | b"mv" | b"cp" | b"drift" | b"cc-hello" | b"tcc" | b"lua" | b"micropython" | b"qjs" | b"curl" | b"cares-test" | b"ffi-test" | b"wl-test" | b"xkb-test" | b"vterm-test" | b"ft-test" | b"wlclient" | b"oxterm" | b"sysmon" | b"doom" | b"havoc" | b"xwayland" | b"xclient" | b"xcbdemo" | b"xlibdemo" | b"xeyes" | b"twm" | b"xterm" | b"jail" | b"fstest"
        ) {
        ...
        let budget = if cmd == b"shell" {
            ...
            384 * 1024 * 1024 // libwayland + shm + funds children (oxterm + havoc + Xwayland at boot)
        } else if cmd == b"gpu" {
        ...
        if cmd == b"oxcomp" {
            ...
            for (name, handle) in [
                ...
                (b"xeyes".as_slice(), oxbow_abi::BOOT_IMG_XEYES),
                (b"twm".as_slice(), oxbow_abi::BOOT_IMG_TWM),
                (b"xterm".as_slice(), oxbow_abi::BOOT_IMG_XTERM),
            ] {
