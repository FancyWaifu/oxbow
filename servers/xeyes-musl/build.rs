...
fn main() {
    ...
    let (x11, xcb, xcbproto, xau, xp, xext, ice, sm, xt, xmu, xeyes) = (
        ...
        pp("libXmu-1.2.1"), pp("xeyes-1.1.2"));
    ...
    // xeyes: NO XRENDER (use plain Xlib drawing — avoids the xrender/xcb-present chain)
    w(format!("{xeyes}/config.h"), "/* oxbow xeyes config: plain Xlib (no XRENDER) */\n");
    ...
    // xeyes (the upstream app) + the musl personality
    let mut ba = base();
    ba.define("OXBOW_ARGV0", "\"xeyes\"")
      .include(&xeyes).include(&pers)
      .include(format!("{xt}/include")).include(format!("{xt}/include/X11"))
      .include(format!("{xext}/include")).include(format!("{xmu}/include"))
      .include(format!("{sm}/include")).include(format!("{ice}/include"));
    shared(&mut ba);
    for f in ["Eyes","transform","xeyes"] { ba.file(format!("{xeyes}/{f}.c")); }
