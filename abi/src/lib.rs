...
pub const SYS_SET_FSBASE: u64 = 63; // (addr) -> 0. Set the calling thread's FS base
...
// NOTE: handles 48-52 are DATA handles (BOOT_GPU_FB=50, BOOT_GPU_CURSOR=51,
// BOOT_SESSION_CHAN=52) — image handles must NOT reuse them. TWM=52 once collided
// with BOOT_SESSION_CHAN, overwriting the shell's session-channel side and killing
// graphical login. The X-demo images live at 53+ to stay clear.
pub const BOOT_IMG_XCLIENT: Handle = 53; // xclient — minimal raw-X11 client (loopback TCP -> Xwayland)
pub const BOOT_IMG_XCBDEMO: Handle = 54; // xcbdemo — X client on real libxcb (loopback TCP -> Xwayland)
