// Adapted from rust-lang/rust library/std/src/thread/tests.rs for the oxbow libtest
// harness. Stresses oxbow's thread backend (spawn/join/detach-reap), park/unpark +
// park_timeout (futex parker), panic propagation across join (panic=unwind), thread
// names (stored Rust-side; set_name is a no-op on oxbow), ThreadId, and scoped threads.
//
// crate:: -> std::, super::sleep -> thread::sleep. EXCLUDED:
//   * test_thread_os_id_not_equal — calls thread::current_os_id(), which std re-exports
//     pub(crate) (not reachable from an external crate; the backend does return the real
//     kernel tid via SYS_THREAD_ID, so the behavior is fine — only the API is internal).
//   * test_named_thread_truncation (#[cfg(linux gnu / apple)], uses libc) — N/A on oxbow.
//   * test_minimal_thread_stack (#[cfg(windows)]) / scope_join_race (#[cfg(miri)]).
#![no_main]
