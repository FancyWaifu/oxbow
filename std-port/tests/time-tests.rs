// Adapted from rust-lang/rust library/std/tests/time.rs for the oxbow libtest harness.
// Exercises oxbow's clock backend: Instant (SYS_UPTIME_MS, monotonic) and SystemTime
// (CMOS RTC via SYS_WALLTIME). The #[cfg(unix)] max-range test is excluded (oxbow is not
// unix). instant_monotonic_concurrent's per-thread count is reduced from 5_000_000 to
// 100_000 so 8 threads complete in reasonable time under QEMU (still a real cross-thread
// monotonicity stress: ~800K Instant::now() syscalls).
#![no_main]
