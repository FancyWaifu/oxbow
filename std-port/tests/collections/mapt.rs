...
use std::hash::{BuildHasher, BuildHasherDefault, DefaultHasher, RandomState};
use crate::test_rng;

// https://github.com/rust-lang/rust/issues/62301
fn _assert_hashmap_is_unwind_safe() {
    fn assert_unwind_safe<T: std::panic::UnwindSafe>() {}
    assert_unwind_safe::<HashMap<(), std::cell::UnsafeCell<()>>>();
}
...
mod test_extract_if {
    ...
    fn consuming_nothing() {
        ...
        assert!(map.extract_if(|_, _| false).eq_sorted(std::iter::empty()));
        assert_eq!(map.len(), 3);
    }
