...
main(int argc, char *argv[])
{
	...
	if (!argc) {
		...
		for (newline = 0; *argv; argc--, argv++) {
			...
			if (many)
				printf("%s==> %s <==\n", newline ? "\n" : "", *argv);
			/* oxbow port: fstat() is only needed for -f (follow); short-circuit
			 * so plain `tail` works while libc stat is still a stub. */
			if (fflag && fstat(fd, &st1) < 0)
				eprintf("fstat %s:", *argv);
			if (fflag && !(S_ISFIFO(st1.st_mode) || S_ISREG(st1.st_mode)))
				fflag = 0;
			newline = 1;
			if (tail(fd, *argv, n) < 0) {
				ret = 1;
