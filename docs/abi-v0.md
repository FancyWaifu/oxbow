...
## 41. curl — real libcurl on oxbow (v1-curl)

curl runs on oxbow and fetches HTTP URLs over TCP:
```
oxbow:/$ curl http://10.0.2.2:8080/
It works! curl on oxbow fetched this over TCP.
```
This is the biggest port: curl 8.11.1's libcurl (132 `.c` files), HTTP-only,
**no TLS** (plain HTTP — a homelab LAN server is the use case). Built on the BSD
sockets shim (§40); a small `oxmain.c` drives `curl_easy_perform`. `curl` shell
command, `BOOT_IMG_CURL=27`.

### 41.1 Configure, don't hand-write
The fiddly `curl_config.h` (recv/send arg types, `SIZEOF_*`, `HAVE_*`) was
generated by running curl's `./configure --without-ssl --disable-<everything but
http>` on the host, then adapting it (disable ALTSVC/HSTS/AWS/NETRC/DOH which
pull `sscanf`/special deps; add `__oxbow__` so `curl.h` includes `<sys/select.h>`;
`NO_TM_GMTOFF`; `-Dalloca=__builtin_alloca`).

### 41.2 libc/headers it forced (reusable)
~15 POSIX headers (`sys/types.h`, `sys/stat.h`, `sys/select.h`, `sys/uio.h`,
`poll.h`, `net/if.h`, `pwd.h`, `libgen.h`, …) + functions: `inet_ntop`,
`getsockname`/`getpeername`/`bind`/`ioctl`/`getsockopt`, `strcspn`/`strtok_r`/
`strerror_r`/`basename`/`fseeko`, `arc4random`, `stat`/`fstat`/`pipe`/`rename`
stubs, more `errno`/fcntl flags.

### 41.3 The four bugs (debugged with CURLOPT_VERBOSE)
1. spawned programs had no net cap → granted `BOOT_NET_EP` (§40).
2. `getsockname` returned family 0 → curl's `inet_ntop` failed; set `AF_INET`.
3. `getsockopt(SO_ERROR)` left the value buffer untouched → curl read garbage;
   write 0.
4. **The decisive one:** `poll`/`select` echoed the *requested* events to
